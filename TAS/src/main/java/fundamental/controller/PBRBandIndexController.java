...
@Controller
public class PBRBandIndexController extends HttpServlet{
	@Autowired
	FundamentalService service;
	
	@RequestMapping(value = "/pbrbandindex.do", method = RequestMethod.GET)
	public ModelAndView list(String code){
		...
		BpsDTO 					 bps 		       = service.bps(enterpriseFinance, codeList.get(15).getTotalstock(), codeList.get(0).getCode());
		...
		mav.setViewName("chart_pbr_layout");
		return mav;
	}
}
